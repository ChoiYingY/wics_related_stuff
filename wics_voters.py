...
def validate_file_columns(filename, dataframe):
    ...
    for column in required_columns:
        if column not in dataframe:
            raise Exception(f"'{column}' column not found from file named '{filename}'.\n-> Possible reason: the column might have extra space(s) / capitalization problems in name, or it simply has a different name (e.g. Email Address, Academic Year).")
...
def get_emails_from(file):
    ...
    df = pd.read_excel(file)
    if 'Email' not in df:
        raise Exception(f"'Email' column not found from file named '{file}'.\n-> Possible reason: the column might have extra space(s) / capitalization problems in name, or it simply has a different name (e.g. Email Address).")
